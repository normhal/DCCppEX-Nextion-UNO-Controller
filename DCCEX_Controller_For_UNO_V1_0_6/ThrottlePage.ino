...
void throttlePage(uint8_t button)
{
  switch(button)                  
  {
    ...
    case SliderEvent:
    {
      int8_t response = getNextionValue("S1");
...
//      guestSpeed = 0;
    ...
    default:                                                  
    {
      if((button >= TabSlotStart) && (button < (TabSlotStart + locosPerPage)))      //Process the 10 Tabs
      {
        if(guestActive == true)
        {
          nextionCommand("AD.bco=" + String(GREY));
...
//  encoderPos = locos[selectedIDs[slot]].speed;
...
{
  for(uint8_t l_fSlot=0; l_fSlot<10; l_fSlot++)       //10 function slots representing 2 uint8_ts per group and on a page - never more:-)
  {
